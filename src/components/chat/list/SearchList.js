...
export const SearchList = ({ result, isSearching, searchTerm, setSelectedUser, setSearch, setIsSearching, setSearchResult, setComponentType }) => {
						...
						<>
							{ result.map((user) => (
								<div className="search-result-container-item" key={user._id} onClick={() => addUsernameToUrlQuery(user)}>
									<Avatar name={user?.username} bgColor={user?.avatarColor} size={40} textColor="#ffffff" avatarSrc={user?.profilePicture} />
